...
import {
  extractXmlTag,
  escapeXml,
  extractGovTalkErrorMessage,
} from "@/server/hmrc/ct600/xml-utils";

export { extractGovTalkErrorMessage as extractRtiErrorMessage };
...
export async function submitAndPollRtiXml(opts: {
  ...
  const errorText = extractGovTalkErrorMessage(receipt);
