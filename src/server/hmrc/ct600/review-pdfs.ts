...
import { penceToPoundsDisplay } from "@/server/hmrc/ct600/xml-utils";
...
function money(pence: number) {
  return penceToPoundsDisplay(pence);
}
...
function formatIsoDateBoxes(iso: string) {
  const m = /^(\d{4})-(\d{2})-(\d{2})$/.exec(iso);
  if (!m) return iso;
  return `${m[3]} ${m[2]} ${m[1]}`;
}
...
function drawHeader(page: PDFPage, fonts: Fonts, title: string, subtitle: string) {
  page.drawText(title, { x: 40, y: PAGE.h - 42, size: 14, font: fonts.bold, color: INK });
  ...
  page.drawLine({
    start: { x: 40, y: PAGE.h - 64 },
    end: { x: PAGE.w - 40, y: PAGE.h - 64 },
    thickness: 1,
    color: RULE,
  });
}
...
function boxRow(
  ...
  page.drawText(box, { x: 40, y, size: 8, font: fonts.bold, color: MUTED });
  page.drawText(label, { x: 72, y, size: 9, font: fonts.regular, color: INK });
  page.drawText(value, {
    x: PAGE.w - 40 - fonts.bold.widthOfTextAtSize(value, 10),
    y,
    size: 10,
    font: fonts.bold,
    color: INK,
...
function section(page: PDFPage, fonts: Fonts, y: number, title: string) {
  page.drawText(title, { x: 40, y, size: 11, font: fonts.bold, color: INK });
  return y - 18;
}
...
/**
 * Filled CT600(2025)-style review PDF using company credentials + return figures.
 * Layout mirrors HMRC CT600 box structure (software substitute — not redistributed stationery).
 */
export async function buildCt600FormReviewPdf(opts: {
  ...
  const costs =
    Number(f.costOfSalesPence) + Number(f.administrativeExpensesPence);
  const otherIncome = Number(f.otherIncomePence);
  const fyStart = Number(f.periodStart.slice(0, 4));
  const fyEnd = Number(f.periodEnd.slice(0, 4));

  // ——— Page 1: company + period ———
  {
    ...
    drawHeader(
      ...
      "HydraTax filled return for review · submit online via CT Online XML",
    );
    let y = PAGE.h - 88;
    y = section(page, fonts, y, "Company information");
    y = boxRow(page, fonts, y, "1", "Company name", company.name.toUpperCase());
    y = boxRow(
      page,
      fonts,
      y,
    ...
    y = boxRow(page, fonts, y, "3", "Tax reference (UTR)", company.utr || "—");
    y = boxRow(page, fonts, y, "4", "Type of company", "0");
    y -= 8;
    if (company.registeredOffice) {
      page.drawText("Registered office", {
        x: 40,
        y,
        size: 8,
        font: fonts.bold,
      ...
      y -= 12;
      for (const line of company.registeredOffice.split(/,\s*/).filter(Boolean)) {
        page.drawText(line, { x: 40, y, size: 9, font: fonts.regular, color: INK });
        y -= 12;
      }
      y -= 6;
    }
    y = section(page, fonts, y, "About this return");
    y = boxRow(
      page,
      fonts,
      y,
      ...
      formatIsoDateBoxes(f.periodStart),
    );
    y = boxRow(
      page,
      fonts,
      ...
      formatIsoDateBoxes(f.periodEnd),
    );
    y -= 6;
    y = boxRow(page, fonts, y, "40", "A repayment is due for this return period", "");
    y = boxRow(page, fonts, y, "55", "This return contains estimated figures", "");
    y -= 10;
    y = section(page, fonts, y, "Accounts and computations");
    y = boxRow(
      page,
      fonts,
      y,
    ...
    page.drawText(
      `Directors: ${(company.directors ?? []).filter(Boolean).join(", ") || "—"}`,
      { x: 40, y: 56, size: 8, font: fonts.regular, color: MUTED },
    );
  }
  ...
  {
    const page = doc.addPage([PAGE.w, PAGE.h]);
    drawHeader(page, fonts, "CT600 — Income and tax calculation", company.name);
    let y = PAGE.h - 88;
    y = section(page, fonts, y, "Tax calculation — Turnover");
    y = boxRow(page, fonts, y, "145", "Total turnover from trade", `£ ${money(turnover)}`);
    y = boxRow(
      page,
      fonts,
      y,
    ...
    y = boxRow(
      ...
      y,
      "170",
      "Bank / building society / other interest",
      `£ ${money(otherIncome)}`,
    );
    y = boxRow(
      page,
    ...
    y -= 10;
    y = section(page, fonts, y, "Tax calculation");
    y = boxRow(page, fonts, y, "326", "Number of associated companies in this period", "0");
    y = boxRow(
      page,
      fonts,
      y,
      "330–345",
      `Financial year ${fyStart} · rate 19%/25% as applicable`,
      `£ ${money(opts.taxChargePence)}`,
    );
    if (fyEnd !== fyStart) {
      y = boxRow(
        page,
    ...
    y -= 10;
    page.drawText(
      `Deductible costs (memo): cost of sales + admin = £ ${money(costs)}`,
      { x: 40, y, size: 8, font: fonts.regular, color: MUTED },
    );
  }

  // ——— Page 3: BS + declaration ———
  {
    const page = doc.addPage([PAGE.w, PAGE.h]);
    drawHeader(page, fonts, "CT600 — Balance sheet figures & declaration", company.name);
    let y = PAGE.h - 88;
    y = section(page, fonts, y, "Balance sheet extracts (from attached accounts)");
    y = boxRow(
      page,
      fonts,
      y,
      "",
      "Tangible assets",
      `£ ${money(Number(f.tangibleAssetsPence))}`,
    );
    y = boxRow(
      page,
      fonts,
      ...
      `£ ${money(Number(f.cashAtBankPence))}`,
    );
    y = boxRow(page, fonts, y, "", "Debtors", `£ ${money(Number(f.debtorsPence))}`);
    y = boxRow(
      page,
      fonts,
      y,
    ...
    y = boxRow(
      ...
      `£ ${money(Number(f.calledUpShareCapitalPence))}`,
    );
    y = boxRow(
      page,
      fonts,
      ...
      `£ ${money(Number(f.profitAndLossAccountPence))}`,
    );
    y -= 16;
    y = section(page, fonts, y, "Declaration");
    page.drawText(
    ...
    y = boxRow(
      ...
      "985",
      "Status",
      company.declarantStatus?.trim() || "Director",
    );
    page.drawText(
      "This is a HydraTax software substitute of CT600 for client review. The live submission uses CT Online XML with iXBRL accounts and computations.",
      {
        x: 40,
        y: 48,
        size: 7,
